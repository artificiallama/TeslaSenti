...
The total number of texts are divided into two groups - 67% for training and 33% for testing (hold out set). The 67% is used for K-fold cross-validation (CV). The hyperparameter *alpha* is tuned using CV. The GridSearchCV() function is used to identify the best value of alpha amongst (0.1, 0.5, 1.0, 3.0, 5.0, 10.0, 50.0, 1e2, 1e3, 1e4).  The mean train and test accuracies from the CV are shown in the graph below. The highest mean test accuracy of 0.647 is realized for alpha=1.0. The corresponding mean train accuracy is 0.76. For alpha=0.0 the train and test accuracy is 0.81 and 0.64. Increasing the value of alpha to 1.0 decreases the overfit but further increasing alpha decreases the accuracy. This is because higher values of alpha lead to underfitting. The test accuracy is a constant for alpha between 1.0 and ~4.0. Hence the model with any value of alpha between 1.0 and 4.0 can be saved and used for inference in the deployed model. The overfitting is lower for alpha=4.0 compared to alpha=1.0

<p align="left">
<img  width="350" height="250" src="images/CV_accuracy_laplace.png">

<img  width="350" height="250" src="images/CV_accuracy_laplace_10.png">
</p>
...
Experiments were carried out with two more models - LinearSVC and LogisticRegression [[12]](#12). However the results (in terms of test metrics and also overfitting) obtained were similar to those obtained using Naive Bayes. An important avenue that could be explored is using a deep learning model such as BERT [[20]](#20). However, typically deep learning models are data hungry and therefore the current size of training data might not suffice. 
...
The accuracies and confusion table presented above correspond to using all the featues. In the next section feature selection is explored.

## Feature selection

For a given problem, all features (i.e. predictors) may not be equally important. It is possible to identify a subset of most important features and use them for model building rather than use all the available features. This decreases the overfitting and can potentially improve accuracy. It also decreases the computational burden thereby leading to more efficient training and model inference. While using cross-validation (CV) it is important to implement the feature selection within the folds of CV. It is incorrect to carry out feature selection using all the data and then use the most important subset of features to do the hyperparameter tuning using CV ([[25]](#25), [[26]](#26)). This is because information is leaked from the training to the testing data set since feature selection is based on all the data. The correct procedure is to include feature selection in the CV. In 5-fold CV, feature selection is done using only 4/5 th of the (training) data for any given fold.

The *pipeline* of transforms ([[27]](#27)) is used to combine feature selection with the identification of the best value of Laplace smoothing parameter within CV. Feature selection results in the decrease of vocabulary size. The figures below show the results of feature selection within CV folds using Chi2 [[12]](#12). Each color is a different value of top N features used. The solid curves show the train accuracy while the dashed curve show the test accuracy. The right panel is same as left panel except that only lower values of alpha are shown on the x-axis. In general as the value of alpha increases both the train and test accuracies decrease. Also both the train and test accuracies increase as the number of features selected increase. The overfitting, quantified by the gap between solid and dased curve for a given value of topN, is the least for topN=100. However both the train and test accuracy is very low (~0.53) for topN=100. This is because of high bias due to very few degrees of freedom. Consider the right panel which shows the results for alpha between 0.1 and 3.0. For a given value of topN the test accuracy maximizes and remains constant between 1.0 and 3.0. This highest test accuracy is about 0.64, at alpha=1.0 for topN=12000. As topN is decreased from 12000 to 1000, the test accuracy decreases from 0.64 to ~0.62. The overfitting is least for topN=1000. However the test accuracy is low (~0.62). A value of topN between 2000 and 8000 can be good compromise between lower overfitting and high test accuracy compared to using topN=12000. For example for topN=4000, the gap between the green curves has decreased substantionally compared to the black curves while the test accuracy is almost the same as using all the features (black curve). Therefore, for this particular problem using a subset of top 4000 most important features decerases the overfitting while maintaining the same test accuracy as using all the features.

<p align="left">
<img  width="450" height="250" src="images/chi2_featureselect_laplace.png">

<img  width="450" height="250" src="images/chi2_featureselect_laplace_few.png">
</p>
