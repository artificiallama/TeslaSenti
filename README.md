...
For a given problem, all features (i.e. predictors) may not be equally important. It is possible to identify a subset of most important features and use them for model building rather than use all the available features. This decreases the noise leading to decrease in overfitting and also decreases the computational burden. While using cross-validation (CV) it is important to implement the feature selection within the folds of CV. It is incorrect to carry out feature selection using all the data and then use the most important subset of features to do the hyperparameter tuning using CV ([[25]](#25), [[26]](#26)). This is because information is leaked from the training to the testing data set since feature selection is based on all the data. The correct procedure is to include feature selection in the CV. In 5-fold CV, feature selection is done using only 4/5 th of the (training) data for any given fold.
